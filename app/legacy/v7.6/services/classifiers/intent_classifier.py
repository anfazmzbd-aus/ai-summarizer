...
def classify_intent(text):
    ...
    business_words = ["revenue", "profit", "market", "sales", "%"]

    meeting_words = ["meeting", "agenda", "should", "must", "follow up"]

    research_words = ["research", "study", "analysis", "result"]
    ...
    if detected_intents:
        primary_intent = max(detected_intents, key=lambda x: scores[x])

    else:
        primary_intent = "fallback"
        detected_intents = ["fallback"]

    logger.info(f"****INTENT SCORES: {scores}")

    logger.info(f"****PRIMARY INTENT: {primary_intent}")

    logger.info(f"****SELECTED INTENTS: {detected_intents}")

...
def run_graph(state):
    ...
    intent_info = classify_intent(state["text"])
    logger.info(f"****agent_graph intent_info : {intent_info}")
    # --------------------------------------------------
    # Strategy Builder
    # --------------------------------------------------

    strategy = build_strategy(intent_info["intents"])

    logger.info(f"****agent_graph STRATEGY : " f"{strategy}")
    # --------------------------------------------------
    # Semantic Router
    # --------------------------------------------------

    plan = semantic_router(state["text"], intent_info, strategy)
    ...
    execution_plan = Scheduler.build_execution_plan(
        plan["selected_agents"], AGENT_REGISTRY
    )

    plan["selected_agents"] = execution_plan["selected_agents"]

    plan["execution_order"] = execution_plan["execution_order"]

    plan["parallel_groups"] = execution_plan["parallel_groups"]

    plan.setdefault("metadata", {})

    plan["metadata"].update(execution_plan["metadata"])
    ...
    execution_metadata = {
        ...
        "agent_count": 0,
        # only DAG groups
        "parallel_groups": [g for g in groups if "summary" not in g],
        "timings": {},
        "trace_count": 0,
        # NEW
        "execution_events": [],
    ...
    if PREPROCESSING_NODE in plan["selected_agents"]:
        ...
        agent_info = AGENT_REGISTRY[PREPROCESSING_NODE]

        agent = agent_info["function"]

        logger.info("****PREPROCESS START: summary")
        ...
        timings[PREPROCESSING_NODE] = duration

        execution_metadata["preprocessing"] = {
            "agent": PREPROCESSING_NODE,
            "duration": duration,
            "output_size": len(state.get("summary", "")),
        }

        logger.info(f"****PREPROCESS END: {duration}s")
    ...
    for group_index in range(1, len(groups)):
        ...
        parallel_agents = groups[group_index]
        ...
        for agent_name in parallel_agents:
            ...
            tasks.append((run_agent, (agent_name, agent, state)))

        executor = DAGExecutor()

        state, failed, results = executor.execute_group(tasks, state)
        ...
        if failed:
            ...
            state = executor.retry_failed(tasks, state, failed)

        # ensure retry results are counted
        for agent in failed:
            execution_metadata["agents_executed"].append(agent)

        agent_order_map = {agent: i for i, agent in enumerate(groups[group_index])}

        results = sorted(results, key=lambda r: agent_order_map.get(r["agent"], 999))
        results = stabilize_parallel_order(results, groups[group_index])
        logger.info(f"****GROUP {group_index} RESULTS" f"{results}")
        ...
        for result in results:
            ...
            execution_metadata["agents_executed"].append(agent_name)
            execution_metadata["execution_events"].append(
                {
                    "agent": agent_name,
                    "stage": "dag",
    ...
    logger.debug(f"****TRACE SAMPLE: " f"{traces[-1] if traces else None}")
    ...
    execution_metadata["agent_count"] = len(execution_metadata["agents_executed"]) + (
        1 if "summary" in plan["selected_agents"] else 0
    )

    execution_metadata["timings"] = timings

    execution_metadata["total_execution_time"] = round(
        time.perf_counter() - graph_start, 6
    )
    ...
    execution_metadata["trace_count"] = len(traces)

    traces = trace_logger.get_traces()

    execution_metadata["trace_count"] = len(traces)

    execution_metadata["trace_sample"] = traces[-1] if traces else None

    execution_metadata["retry_count"] = len(failed)

    execution_metadata["failed_agents"] = failed
    ...
    state["snapshot"] = RuntimeSnapshot.create(state)

    state["metrics"] = RuntimeMetrics.collect(state)

    state["graph"] = GraphExporter.export(state)

    runtime_cache.clear()

    runtime_cache.update(state["metrics"])

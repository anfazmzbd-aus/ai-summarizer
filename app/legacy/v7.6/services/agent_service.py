...
def run_agents(text, summary_length):
    ...
    logger.info(f"****REGISTERED AGENTS: " f"{list(AGENT_REGISTRY.keys())}")
...
def extract_actions(text: str):
    ...
    return [
        line.strip()
        for line in lines
        if "should" in line.lower() or "must" in line.lower() or "need" in line.lower()
    ]


def extract_insights(text: str):

    return ["Business context detected", "Key operational information identified"]


def extract_findings(text: str):

    return ["Document analyzed", "No anomalies detected"]

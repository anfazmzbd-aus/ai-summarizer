    ...
    def retry_failed(self, group_tasks, state, failed_agents):
        ...
        for task in group_tasks:
            ...
            if agent_name in failed_agents:
                ...
                retry_tasks.append((fn, (agent_name, agent_func, deepcopy(state))))

        if not retry_tasks:
            return state

        logger.info(f"****RETRYING NODES: {failed_agents}")
        ...
        retry_input = deepcopy(state)

        retry_results = execute_parallel(
            [(t[0], (t[1][0], t[1][1], retry_input)) for t in retry_tasks]
        )

        retry_state = {"artifacts": {}}

        for r in retry_results:

            retry_state.setdefault("artifacts", {})

            retry_state["artifacts"].update(r.get("artifacts", {}))

        state["execution"]["retry_count"] += 1

        state["execution"]["failed_agents"] = failed_agents

        return merge_state(state, retry_state, overwrite_agents=failed_agents)

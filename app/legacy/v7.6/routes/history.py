...
def history(request: Request):
    ...
    for row in rows:
        ...
        try:
            output = json.loads(row.agent_output or "{}")
        ...
        summaries.append(
            {
                ...
                "plan": json.dumps(
                    json.loads(row.execution_plan) if row.execution_plan else {},
                    indent=2,
                    ensure_ascii=False,
                ),
                "execution": json.dumps(
                    (
                        json.loads(row.execution_metadata)
    ...
    return templates.TemplateResponse(
        request=request, name="history.html", context={"summaries": summaries}
    )
